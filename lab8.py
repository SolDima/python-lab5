...
# Функція для пошуку студентів за курсом
def find_students_by_course(course_number):
    found = False
    print(f"Студенти на курсі {course_number}:")
    for group, students in students_group.items():
        for student_id, student_info in students.items():
            if student_info['Курс'] == course_number:
                print(f"  Група: {group}, ID: {student_id}, ПІБ: {student_info['ПІБ']}")
                found = True
    if not found:
...
# Пошук студентів на курсі 2
find_students_by_course(2)
